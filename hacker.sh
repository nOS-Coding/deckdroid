#!/usr/bin/env bash
# TermuxDeck OS — Hacker Profile installer
set -euo pipefail
HACKER_PKGS=(nmap netcat-openbsd curl wget tcpdump tshark whois dnsutils)
echo ""
echo "TermuxDeck OS — Hacker Profile"
echo "────────────────────────────────────────────────"
...
echo "$(date -u +%FT%TZ) disclaimer accepted" >> "$HOME/.termuxdeck/logs/disclaimer.log"
for p in "${HACKER_PKGS[@]}"; do
  pkg install -y "$p" 2>/dev/null && echo "  ✓ $p" || echo "  ✗ $p (unavailable on $(uname -m))"
done
python3 "$HOME/.termuxdeck/tools/tdeckconf" set profiles hacker 2>/dev/null || true
